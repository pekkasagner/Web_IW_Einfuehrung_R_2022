...
+ Funktionen haben drei große Vorteile gegenüber Copy-and-paste:
  ...
  2. Wenn die Anforderungen an Code sich verändern, müssen wir nur an einer Stelle etwas ändern, anstatt an vielen Stellen.
...
+ Bei der explorativen Datenanalyse, zum Beispiel beim Erstellen von deskriptiven Statistiken, wenden wir häufig die gleichen Funktionen an.
...
+ Wir könnten den oberen Code Copy-pasten und `continent` durch `country` ersetzen. Oder wir schreiben eine Funktion.
...
+ Die Funktion funktioniert jetzt mit der Variable `country`...
